...
const SONG_DETAILS: Record<string, SongDetail> = {
  ...
  'song-5': {
    ...
    lyrics: [
      { time: 0, text: '你终将会找到属于自己的月亮' },
      { time: 18, text: '在最暗的夜里 也不要慌张' },
      { time: 36, text: '把眼泪藏进风里 继续歌唱' },
      { time: 54, text: '总有人在远方 为你点亮' },
      { time: 72, text: '走过漫长孤独的街巷' },
      { time: 90, text: '你会听见心底温柔回响' },
      { time: 108, text: '每一次跌倒 都不是白忙' },
      { time: 126, text: '都在把你带向 更亮的地方' },
      { time: 144, text: '当清晨慢慢爬上窗台' },
      { time: 162, text: '你会看见 梦正在发光' },
      { time: 180, text: '请相信你所有的倔强' },
      { time: 198, text: '都会被世界好好收藏' },
      { time: 216, text: '哪怕此刻仍有一点迷惘' },
      { time: 234, text: '也别放弃心中的方向' },
      { time: 252, text: '你终将会找到属于自己的月亮' },
      { time: 270, text: '在你的天空里静静照亮' },
    ],
    durationSeconds: 307,
  },
};
